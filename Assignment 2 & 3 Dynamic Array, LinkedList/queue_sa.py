...
# Description: Assignment 3 - Queue Static Array
...
class Queue:
    ...
    def enqueue(self, value: object) -> None:
        """
        This method adds a new value to the end of the queue, implemented with O(1) amortized runtime complexity.
        """
    ...
    def dequeue(self) -> object:
        ...
        self._current_size -= 1
        object = self._sa.get(self._front)
    ...
    def _double_queue(self) -> None:
        """
        This method resizes the original static array by doubling the size
        """
        size = self._sa.length()
        resized_sa = StaticArray(size * 2)

        resized_sa_index = 0

        for i in range(self._front, size):
            item = self._sa.get(i)
            resized_sa.set(resized_sa_index, item)
            resized_sa_index += 1

        if self._back < self._front:
            for j in range(self._back + 1):
                item = self._sa.get(j)
                resized_sa.set(resized_sa_index, item)
                resized_sa_index += 1
...
if __name__ == "__main__":
    ...
    q = Queue()
    print(q)
    for value in [2, 4, 5, 6, 7, 8, 9, 10, 11]:
        q.enqueue(value)
    print(q)
